<template>
    <div class="product-item" @click="showProductDetails(product)">
      ...
      <h2>{{ product.title }}</h2>
      <p class="product-description" v-if="!isExpanded(product.id)">{{ truncatedDescription(product.description) }}</p>
      <p v-else>{{ product.description }}</p>
      
      <button class="ghost-button" v-if="isTruncated(product.description)" @click.stop="toggleExpanded(product.id)">
          {{ isExpanded(product.id) ? 'Read less' : 'Read more' }}
      </button>

      <button class="ghost-button" @click.prevent.stop="addToCart">Add to Cart</button>
